...
<pre><code class="lang-js"><code class="source-code prettyprint">var Foo = sequelize.define(&apos;foo&apos;, {
...
<p>The comment option can also be used on a table, see <a href="#configuration">model configuration</a></p>
<h2 id="data-types">Data types</h2>
<p>Below are some of the datatypes supported by sequelize. For a full and updated list, see <a href="./manual/../api/datatypes">DataTypes</a>.</p>
<pre><code class="lang-js"><code class="source-code prettyprint">Sequelize.STRING                      // VARCHAR(255)
...
<p>The BLOB data type allows you to insert data both as strings and as buffers. When you do a find or findAll on a model which has a BLOB column. that data will always be returned as a buffer.</p>
...
<pre><code class="lang-js"><code class="source-code prettyprint">require(&apos;pg&apos;).types.setTypeParser(1114, function(stringValue) {
  return new Date(stringValue + &quot;+0000&quot;);
  // e.g., UTC offset. Use any offset that you would like.
});</code>
</code></pre>
<p>In addition to the type mentioned above, integer, bigint, float and double also support unsigned and zerofill properties, which can be combined in any order:
Be aware that this does not apply for PostgreSQL!</p>
<pre><code class="lang-js"><code class="source-code prettyprint">Sequelize.INTEGER.UNSIGNED              // INTEGER UNSIGNED
...
<pre><code class="lang-js"><code class="source-code prettyprint">// for enums:
...
<p>Since range types have extra information for their bound inclusion/exclusion it&apos;s not
...
<pre><code class="lang-js"><code class="source-code prettyprint">// defaults to &apos;[&quot;2016-01-01 00:00:00+00:00&quot;, &quot;2016-02-01 00:00:00+00:00&quot;)&apos;
...
<p>However, please note that whenever you get back a value that is range you will
receive:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">// stored value: (&quot;2016-01-01 00:00:00+00:00&quot;, &quot;2016-02-01 00:00:00+00:00&quot;]
range // [Date, Date]
range.inclusive // [false, true]</code>
</code></pre>
<p>Make sure you turn that into a serializable format before serialization since array
extra properties will not be serialized.</p>
<h4 id="special-cases">Special Cases</h4>
<pre><code class="lang-js"><code class="source-code prettyprint">// empty range:
...
<h2 id="deferrable">Deferrable</h2>
<p>When you specify a foreign key column it is optionally possible to declare the deferrable
type in PostgreSQL. The following options are available:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">// Defer all foreign key constraint check to the end of a transaction
...
<p>The last option is the default in PostgreSQL and won&apos;t allow you to dynamically change
the rule in a transaction. See <a href="./manual/transactions/#options">the transaction section</a> for further information.</p>
<h2 id="getters-setters">Getters &amp; setters</h2>
<p>It is possible to define &apos;object-property&apos; getters and setter functions on your models, these can be used both for &apos;protecting&apos; properties that map to database fields and for defining &apos;pseudo&apos; properties.</p>
...
<h3 id="defining-as-part-of-a-property">Defining as part of a property</h3>
<pre><code class="lang-js"><code class="source-code prettyprint">var Employee = sequelize.define(&apos;employee&apos;, {
  name:  {
...
<h3 id="defining-as-part-of-the-model-options">Defining as part of the model options</h3>
...
<h3 id="helper-functions-for-use-inside-getter-and-setter-definitions">Helper functions for use inside getter and setter definitions</h3>
<ul>
<li>retrieving an underlying property value - always use <code>this.getDataValue()</code></li>
</ul>
<pre><code class="lang-js"><code class="source-code prettyprint">/* a getter for &apos;title&apos; property */
function() {
...
<pre><code class="lang-js"><code class="source-code prettyprint">/* a setter for &apos;title&apos; property */
function(title) {
    return this.setDataValue(&apos;title&apos;, title.toString().toLowerCase());
}</code>
</code></pre>
<p><strong>N.B:</strong> It is important to stick to using the <code>setDataValue()</code> and <code>getDataValue()</code> functions (as opposed to accessing the underlying &quot;data values&quot; property directly) - doing so protects your custom getters and setters from changes in the underlying model implementations.</p>
<h2 id="validations">Validations</h2>
<p>Model validations, allow you to specify format/content/inheritance validations for each attribute of the model.</p>
...
<pre><code class="lang-js"><code class="source-code prettyprint">var ValidateMe = sequelize.define(&apos;foo&apos;, {
  foo: {
    type: Sequelize.STRING,
    validate: {
...
<pre><code class="lang-js"><code class="source-code prettyprint">isInt: {
  msg: &quot;Must be an integer number of pennies&quot;
}</code>
</code></pre>
<p>or if arguments need to also be passed add an<code>args</code>property:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">isIn: {
  args: [[&apos;en&apos;, &apos;zh&apos;]],
...
<h3 id="validators-and-allownull-">Validators and <code>allowNull</code></h3>
...
<p>An example:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var Pub = Sequelize.define(&apos;pub&apos;, {
...
<p>In this simple case an object fails validation if either latitude or longitude is given, but not both. If we try to build one with an out-of-range latitude and no longitude, <code>raging_bullock_arms.validate()</code> might return</p>
...
<p>You can also influence the way Sequelize handles your column names:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var Bar = sequelize.define(&apos;bar&apos;, { /* bla */ }, {
...
<p>If you want sequelize to handle timestamps, but only want some of them, or want your timestamps to be called something else, you can override each column individually:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var Foo = sequelize.define(&apos;foo&apos;,  { /* bla */ }, {
...
<p>You can also change the database engine, e.g. to MyISAM. InnoDB is the default.</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var Person = sequelize.define(&apos;person&apos;, { /* attributes */ }, {
  engine: &apos;MYISAM&apos;
})
...
<p>Finally you can specify a comment for the table in MySQL and PG</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var Person = sequelize.define(&apos;person&apos;, { /* attributes */ }, {
...
<pre><code class="lang-js"><code class="source-code prettyprint">// in your server file - e.g. app.js
...
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.import(&apos;project&apos;, function(sequelize, DataTypes) {
  return sequelize.define(&quot;project&quot;, {
    name: DataTypes.STRING,
    description: DataTypes.TEXT
  })
...
<p>Sequelize has built-in support for optimistic locking through a model instance version count.
Optimistic locking is disabled by default and can be enabled by setting the <code>version</code> property to true in a specific model definition or global model configuration.  See <a href="#configuration">model configuration</a> for more details.</p>
...
<p>When starting a new project you won&apos;t have a database structure and using Sequelize you won&apos;t need to. Just specify your model structures and let the library do the rest. Currently supported is the creation and deletion of tables:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">// Create the tables:
Project.sync()
Task.sync()
...
<pre><code class="lang-js"><code class="source-code prettyprint">// Sync all models that aren&apos;t already in the database
sequelize.sync()
...
<p>Because <code>.sync({ force: true })</code> is destructive operation, you can use <code>match</code> option as an additional safety check.
...
<h2 id="expansion-of-models">Expansion of models</h2>
<p>Sequelize Models are ES6 classes. You can very easily add custom instance or class level methods.</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var User = sequelize.define(&apos;user&apos;, { firstname: Sequelize.STRING });
...
<p>Of course you can also access the instance&apos;s data and generate virtual getters:</p>
<pre><code class="lang-js"><code class="source-code prettyprint">var User = sequelize.define(&apos;user&apos;, { firstname: Sequelize.STRING, lastname: Sequelize.STRING });
...
<p>Sequelize supports adding indexes to the model definition which will be created during <code>Model.sync()</code> or <code>sequelize.sync</code>.</p>
