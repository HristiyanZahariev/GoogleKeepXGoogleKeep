...
<div class="content" data-ice="content"><div class="github-markdown">
  ...
  <div data-ice="content"><h1 id="raw-queries">Raw queries</h1>
...
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.query(&quot;UPDATE users SET y = 42 WHERE x = 12&quot;).spread(function(results, metadata) {
...
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.query(&quot;SELECT * FROM `users`&quot;, { type: sequelize.QueryTypes.SELECT})
  .then(function(users) {
    // We don&apos;t need spread here, since only the results will be returned for select queries
  })</code>
</code></pre>
<p>Several other query types are available. <a href="https://github.com/sequelize/sequelize/blob/master/lib/query-types.js">Peek into the source for details</a></p>
<p>A second option is the model. If you pass a model the returned data will be instances of that model.</p>
<pre><code class="lang-js"><code class="source-code prettyprint">// Callee is the model definition. This allows you to easily map a query to a predefined model
...
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.query(&apos;SELECT * FROM projects WHERE status = ?&apos;,
...
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.query(&apos;SELECT * FROM projects WHERE status IN(:status) &apos;,
  { replacements: { status: [&apos;active&apos;, &apos;inactive&apos;] }, type: sequelize.QueryTypes.SELECT }
).then(function(projects) {
  console.log(projects)
})</code>
</code></pre>
<p>To use the wildcard operator %, append it to your replacement. The following query matches users with names that start with &apos;ben&apos;.</p>
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.query(&apos;SELECT * FROM users WHERE name LIKE :search_name &apos;,
...
<p>Bind parameters are like replacements. Except replacements are escaped and inserted into the query by sequelize before the query is sent to the database, while bind parameters are sent to the database outside the SQL query text. A query can have either bind parameters or replacements.</p>
<p>Only SQLite and PostgreSQL support bind parameters. Other dialects will insert them into the SQL query in the same way it is done for replacements. Bind parameters are referred to by either $1, $2, ... (numeric) or $key (alpha-numeric). This is independent of the dialect.</p>
<ul>
...
<pre><code class="lang-js"><code class="source-code prettyprint">sequelize.query(&apos;SELECT *, &quot;text with literal $$1 and literal $$status&quot; as t FROM projects WHERE status = $1&apos;,
